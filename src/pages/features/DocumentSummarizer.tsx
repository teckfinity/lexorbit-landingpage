...
import { FileText, Clock, Layout, Tag, CheckCircle, Users, Building, GraduationCap, Upload } from "lucide-react";
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <div className="w-28 h-28 rounded-3xl mx-auto mb-8 flex items-center justify-center border-2 border-gold/30 shadow-lg" style={{ background: 'linear-gradient(135deg, rgba(212, 165, 116, 0.2), rgba(232, 204, 180, 0.1))', boxShadow: '0 12px 40px rgba(212, 165, 116, 0.2)' }}>
              <FileText className="w-14 h-14 text-gold" />
            </div>
            <h1 className="font-heading text-4xl md:text-5xl lg:text-6xl font-black text-white mb-6">
              Document <span className="text-gold">Summarizer</span>
            </h1>
            <p className="text-xl text-white/85 max-w-2xl mx-auto mb-10">
              Upload any legal document and get a comprehensive IRAC summary in 60 seconds. Extract key terms, holdings, and citations automatically.
            </p>
      ...
      <section className="py-24 px-4 bg-cream-dark">
        <div className="max-w-5xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
            ...
            <p className="text-charcoal/70 text-lg">Upload a document and watch AI summarize it instantly</p>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-white rounded-3xl p-8 md:p-12 shadow-xl">
            <div 
              className={`border-2 border-dashed rounded-2xl p-12 text-center mb-8 transition-all ${isDragging ? 'border-gold bg-gold/5' : 'border-charcoal/20'}`}
              onDragOver={(e) => { e.preventDefault(); setIsDragging(true); }}
              onDragLeave={() => setIsDragging(false)}
              onDrop={() => setIsDragging(false)}
            >
              <Upload className="w-12 h-12 text-gold mx-auto mb-4" />
              <p className="text-lg font-semibold text-navy mb-2">Drop your document here</p>
              <p className="text-charcoal/60">or click to browse (PDF, DOCX, TXT)</p>
            </div>

            <div className="space-y-6">
              <div className="bg-cream rounded-xl p-6 border-l-4 border-gold">
                <div className="flex items-center gap-2 mb-4">
                  <span className="px-3 py-1 bg-gold text-white text-xs font-bold rounded-full uppercase">Issue</span>
                </div>
                <p className="text-charcoal/80">Whether the defendant's actions constituted a breach of fiduciary duty under Delaware law when they failed to disclose material conflicts of interest...</p>
              </div>

              <div className="bg-cream rounded-xl p-6 border-l-4 border-sage">
                <div className="flex items-center gap-2 mb-4">
                  <span className="px-3 py-1 bg-sage text-white text-xs font-bold rounded-full uppercase">Rule</span>
                </div>
                <p className="text-charcoal/80">Under Delaware law, directors owe a fiduciary duty of loyalty requiring full disclosure of material conflicts. See Weinstein Enterprises, Inc. v. Orloff, 870 A.2d 499 (Del. 2005).</p>
              </div>

              <div className="bg-cream rounded-xl p-6 border-l-4 border-navy">
                <div className="flex items-center gap-2 mb-4">
                  <span className="px-3 py-1 bg-navy text-white text-xs font-bold rounded-full uppercase">Application</span>
                </div>
                <p className="text-charcoal/80">The defendant's undisclosed ownership interest in the competing entity created a direct conflict that should have been disclosed prior to the board vote...</p>
              </div>

              <div className="grid grid-cols-3 gap-6 mt-8 pt-8 border-t-2 border-gold/20">
                {[
                  { value: "47 sec", label: "Processing time" },
                  { value: "156 pages", label: "Document length" },
                  { value: "12 citations", label: "Extracted" },
                ].map((stat, i) => (
      ...
      <section className="py-24 px-4 bg-white">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { icon: Clock, title: "60-Second Summaries", desc: "Upload any document up to 500 pages and get a comprehensive IRAC summary in under a minute." },
              { icon: Layout, title: "IRAC Format", desc: "Every summary follows the Issue, Rule, Application, Conclusion format preferred by legal professionals." },
              { icon: Tag, title: "Key Term Extraction", desc: "Automatically identifies and highlights critical legal terms, parties, dates, and holdings." },
            ].map((item, i) => (
              <motion.div 
                key={i} 
      ...
      <section className="py-20 px-4" style={{ background: 'linear-gradient(135deg, #1A2332 0%, #0F1419 100%)' }}>
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-16 text-center">
            {[
              { value: "47s", label: "Avg Summary Time" },
              { value: "500", label: "Max Pages" },
      ...
      <section className="py-24 px-4 bg-cream-dark">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row items-center gap-8 md:gap-0">
            {[
              { num: "1", title: "Upload Document", desc: "Drag and drop any legal document—briefs, contracts, opinions, or pleadings." },
              { num: "2", title: "AI Processes", desc: "Our AI reads and analyzes the document, identifying key legal concepts and structure." },
              { num: "3", title: "Get Summary", desc: "Receive a complete IRAC summary with extracted citations and key terms." },
            ].map((step, i) => (
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { 
                icon: Users, 
                title: "Litigators", 
                desc: "Quickly digest opposing counsel's briefs and motions. Never miss a key argument again.",
                benefits: ["Brief analysis in minutes", "Identify weak arguments", "Prepare faster responses"]
              },
              { 
                icon: Building, 
                title: "Transactional Attorneys", 
                desc: "Review contracts and agreements faster. Extract key terms and obligations automatically.",
                benefits: ["Contract due diligence", "Risk identification", "Clause extraction"]
              },
              { 
                icon: GraduationCap, 
                title: "Law Students", 
                desc: "Master case briefs faster. Understand complex opinions with clear IRAC breakdowns.",
                benefits: ["Better case prep", "Exam preparation", "Research efficiency"]
              },
            ].map((item, i) => (
              <motion.div 
                key={i} 
        ...
        <div className="max-w-3xl mx-auto">
          ...
          >
            ...
            <div className="relative z-10">
              <h2 className="font-heading text-3xl md:text-4xl font-bold text-white mb-4">Stop Reading. Start Understanding.</h2>
              <p className="text-white/85 text-lg mb-10">Join 10,000+ attorneys who summarize documents in seconds with LexOrbit.</p>
